...
resource "aws_route_table_association" "gautam_pub_assoc" {
  count     = var.pub_sn_count
  subnet_id = aws_subnet.gautam_pub_sn.*.id[count.index]
  # subnet_id      = aws_subnet.dev_pub_sn1.id
  route_table_id = aws_route_table.gautam_pub_rt.id
}

resource "aws_internet_gateway" "gautam_igw" {
  vpc_id = aws_vpc.gautam_vpc.id
...
resource "aws_route_table" "gautam_pub_rt" {
  vpc_id = aws_vpc.gautam_vpc.id
...
resource "aws_route" "default_rt" {
  ...
  gateway_id             = aws_internet_gateway.gautam_igw.id
}

resource "aws_default_route_table" "private_rt" {
  #assigning default rt created by vpc to make our own default rt
  default_route_table_id = aws_vpc.gautam_vpc.default_route_table_id

  tags = {
    # Name = "gautam-pr-rt"
    Name = var.gautam-pvt-rt
  }
}

resource "aws_security_group" "gautam-sg" {
...
resource "aws_subnet" "gautam_pub_sn" {
  count                   = var.pub_sn_count
  vpc_id                  = aws_vpc.gautam_vpc.id
...
resource "aws_subnet" "gautam_app_sn" {
...
resource "aws_subnet" "gautam_db_sn" {
...
resource "aws_db_subnet_group" "rds_sng" {
  ...
  subnet_ids = aws_subnet.gautam_db_sn.*.id
  tags = {
    name = "db-sng"
  }
}
