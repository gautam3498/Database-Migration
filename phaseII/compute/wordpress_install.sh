...
sudo mysql -e "CREATE DATABASE gautam_db"
sudo mysql -e "CREATE USER 'gautam_user'@'localhost' IDENTIFIED BY 'p3rsonalPlu5'"
sudo mysql -e "GRANT ALL ON gautam_db.* TO 'gautam_user'@'localhost' IDENTIFIED BY 'p3rsonalPlu5'"
...
sudo sed -i "s/'database_name_here'/'gautam_db'/g" wp-config.php
sudo sed -i "s/'username_here'/'gautam_user'/g" wp-config.php
...
sudo echo 'cowsay "Ubuntu Linux 2 AMI - gautamOfEngineers"' >> /etc/default/motd-news
sudo chmod 755 /etc/update-motd.d/motd-news
cowsay "Ubuntu On-prem Server --- gautam of Engineers"
